...
| [`06-agent-team.md`](06-agent-team.md) | **Phase 3** — the standing agent team (see `agents/` for the current roster) sharing the memory and the task board |
...
**Phase 2** — the optional asoode bridge (outbox out, live socket in). Shipped; asoode's P0 items were fixed on its side (idempotent `externalRef`, description and assignees on create, a change feed, a signed socket ticket).
...
- Inbound arrives over a **live Socket.IO subscription** from an asyncio task in the daemon lifespan,
  authenticated with a short-lived ticket the PAT is exchanged for on every connect. Inbound only
  CREATES until the two-sided conflict policy is decided.
